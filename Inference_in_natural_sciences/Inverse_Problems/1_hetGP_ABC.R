...
library(hetGP)
settings <- list(return.hom = FALSE) # To keep homoskedastic model used for training
model <- mleHetGP(X = matrix(samples$x1, ncol=1), Z = samples$t,
                  lower = rep(0.01, 1), upper = rep(10, 1),
                  covtype = "Matern5_2", settings = settings)

df_ABC <- data.frame(x1=seq(0, 1,, 101))

pred <- predict(model, x=matrix(df_ABC$x1, ncol=1), xprime=matrix(df_ABC$x1, ncol=1))
library(MASS)
n_real <- 100
simu_post_GP <- t(mvrnorm(n=n_real, mu=pred$mean, Sigma=pred$cov))
colnames(simu_post_GP) <- paste0("simu_", seq(n_real))
df_ABC <- cbind(df_ABC, pnorm(treshold-simu_post_GP, sd=sqrt(pred$nugs)))
for(i in seq(n_real)){
  df_ABC[, i+1] <- df_ABC[, i+1] / mean(df_ABC[, i+1])
}
df_ABC$mean_ABCposterior <- as.vector(pnorm(treshold, mean=pred$mean, sd=sqrt(pred$sd2)))
...
#   geom_ribbon(data=data.frame(x1=seq(0, 1,, 101), 
#                               ymin=pred$mean-1.96*sqrt(pred$sd2), 
#                               ymax=pred$mean+1.96*sqrt(pred$sd2)),
...
for(ibatch in seq(nbatch)){
  ...
  if(sampling=="EIV"){
    an <- as.vector((treshold-pred$mean)/(pred$sd2+pred$nugs))
    crit <- -sapply(candidates, function(x){
      indx <- which.min(abs(df_ABC$x1-x))
      vect <- pred$cov[rep(indx, sizebatch), ]
      cn <- diag(t(vect)%*% solve(pred$sd2[indx]+diag(pred$nugs[indx], sizebatch)) %*% vect)
      T1 <- sapply(seq(length(an)), function(i){
        T.Owen(an[i], sqrt(pred$nugs[i]+ pred$sd2[i]-cn[i])/ 
                 sqrt(pred$nugs[i]+ pred$sd2[i]+cn[i]))
      })
      T2 <- sapply(seq(length(an)), function(i){
        T.Owen(an[i], sqrt(pred$nugs[i])/ sqrt(pred$nugs[i]+ 2*pred$sd2[i]))
      })
      return(2*mean(T1-T2))
    })
  ...
  if(sampling=="EIMAD"){
    ...
    crit <- -sapply(candidates, function(x){
      ...
      cn <- diag(t(vect)%*% solve(pred$sd2[indx]+diag(pred$nugs[indx], sizebatch)) %*% vect)
      T1 <- sapply(seq(length(an)), function(i){
        T.Owen(an[i], sqrt(pred$sd2[i]-cn[i])/ 
                 sqrt(pred$sd2[i]+cn[i]))
      })
  ...
  write.csv(res,
            file=paste0("./res/scores/hetGP_sampling_", sampling, "_jobnumber_", job_number, ".txt"))
  
  write.csv(samples,
            file=paste0("./res/samples/hetGP_sampling_", sampling, "_jobnumber_", job_number, ".txt"))
}
