...
# 
# ggplot(df_plot, aes(x=x1, y=t))+
#   geom_raster(aes(fill=pdf))+
#   geom_point(data=samples)

beta <- 0.75
Fit <- run_pCN(starting_point=rnorm(par_basis_functions$order_tot)*starting_sigma,
...
if(abs(Acceptance.Rate-0.25)>0.05){
  while(abs(Acceptance.Rate-0.25)>0.05){
    ...
    Fit <- run_pCN(starting_point=Fit$Posterior1[nrow(Fit$Posterior1), ],
                   samples, Iterations =10000, Thinning = 1, 
                   par_basis_functions=par_basis_functions,
                   name_index=name_index,
                   starting_lengthscale = starting_lengthscale,
...
# ggplot(df_plot, aes(x=x1, y=t, fill=pdf_00080))+
#   geom_raster()+
#   geom_contour(mapping=aes(z=cdf_00080))
# plot(df_plot$cdf_00080[df_plot$t==0.15]/mean(df_plot$cdf_00080[df_plot$t==0.15]), ylim=c(0, 1.2))
df_ABC <- df_plot %>%
